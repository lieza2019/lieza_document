...
\author
{A. Goto,$^{1\ast}$ K. Sakari,$^{1}$ R. Matsuda$^{2}$\\
\\
...
\begin{sciabstract}
  Recent progressive Artificial Inteligence technology brings drastically
...
Matching system on interlocking logic structures.
...
\begin{Corollary} \label{dom_bin_join}w
  ...
  \begin{Proof}
    ...
    \Gamma_1 \bigr\} = \bigl\{ t'' \ |\ (t'', \mathcal{R}'') \in
...
\begin{Definition}[Subterms]
  We define the set of subterms of a given term $t \in \mathbb{T}$ as follow.
  \begin{displaymath}
    \begin{array}{lll}
      \mathpzc{sub}(t) &:=
      & \left\{ \begin{array}{ll}
...
\begin{Lemma}{soundness on extended matching}
  ...
  \begin{Proof}
    ...
    that $\Delta_j \vdash t_j : \mathcal{R}_j$ s.t.
    ...
    \mathpzc{Dom}({\Gamma_2}) \bigr) \Bigr)$ for their size constraints.
    ...
    $t_2 \cdots t_n$ s.t. $\Gamma_1 \vdash t_1: \mathcal{R}_1 \cdots
    \Gamma_1 \vdash t_1: \mathcal{R}_1$ respectively, we can derive
    $\{t_2 \cdots t_n\} \cup \bigcup_{j=2}^n\Gamma_j \vdash
    \{t_2 \cdots t_n\}: {\mathcal{R}_1}^*$ from (T-Cat0-canon).
    Here we let n be the number of subterms of $\{t_1 \cdots t_n\}$, the
    cases of $n \in \{1, 2\}$ are shown as the cases of $j \le 2$ above.
    ...
    which also brings us $\Delta_1 \vdash t_1 : {\mathcal{R}_1}^*$
    with the condition of
    ...
    \bigr) = \phi$ for the term size restriction on
    $\mathpzc{Dom}(\Gamma_1)$, e.g.
    $\{t_2 \cdots t_n\} \not\in \mathpzc{Dom}(\Gamma_1)$.
    From similar reason $t_1 \wedge t_t' \not\in \mathpzc{Dom}(\Gamma_1)
    \cup \bigl(\{t_2 \cdots t_n\} \cup \bigcup_{j=2}^n\mathpzc{Dom}(\Gamma_j)
    \bigr)$ for any $t_t' \in \mathbb{T}$ is also, since
    ...
    \bigcup_{j=2}^n\mathpzc{Dom}(\Gamma_j) \bigr)$, e.g. assumption of
