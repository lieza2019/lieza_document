...
\section*{LIezA, the Logic for Interlocking logics self recognizing
  Architecture}
We implemented to our login on machine using Caml, which is one of the most
...
# term1 (resolv sig_ctrl (typematch (compile "TR_1 & (TR_2 |
NKR_#3) & TLSR_TR4 : (TR | (NKR % RKR)!*)+ & (TLSR % TRSR)*")));;
- : string = "(((TR_1 | {})) & ((TR_2 | NKR_#3)))"
\end{verbatim}
...
And we can see that the last line depicts the subterm bounded to the target
...
\begin{verbatim}
let sig_ctrl = cli_pat (compile "* : (TLSR % TRSR)*");;
val sig_ctrl : Lie_type.pattern = ...
# term1 (resolv sig_ctrl (typematch (compile "TR_1 & (TR_2 |
NKR_#3) & TLSR_TR4 : (TR | (NKR % RKR)!*)+ & (TLSR % TRSR)*")));;
- : string = "TLSR_TR4"
